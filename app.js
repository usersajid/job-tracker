...
function updateCounters() {
  ...
  // Update small text above tabs based on current tab
  if (currentTab === "all") jobCountText.innerText = total;
  else if (currentTab === "interview") jobCountText.innerText = interview;
  else if (currentTab === "rejected") jobCountText.innerText = rejected;
}
...
function renderJobs(filterType) {
  ...
  updateCounters(); // Keep numbers updated
  ...
  if (filteredJobs.length === 0) {
    ...
    filteredJobs.forEach(function(job) {
      ...
      jobCard.innerHTML = `
          <button onclick="deleteJob(${job.id})" class="absolute top-6 right-6 text-slate-400 hover:text-red-500 transition-colors">
          ...
          <div class="flex gap-4">
            <button onclick="changeStatus(${job.id}, 'interview')" class="px-6 py-2.5 text-xs font-bold text-green-600 border border-green-500 rounded hover:bg-green-50 transition-colors tracking-wide">INTERVIEW</button>
            <button onclick="changeStatus(${job.id}, 'rejected')" class="px-6 py-2.5 text-xs font-bold text-red-500 border border-red-500 rounded hover:bg-red-50 transition-colors tracking-wide">REJECTED</button>
          </div>
      `;
...
tabButtons.forEach(button => {
  button.addEventListener("click", function () {
    ...
    tabButtons.forEach(btn => {
      btn.className = "px-8 py-2.5 rounded-lg bg-white text-slate-600 hover:bg-slate-50 font-semibold text-sm transition-colors border border-slate-300";
    });
    // Add active style to clicked button
    this.className = "px-8 py-2.5 rounded-lg bg-[#002C5C] text-white font-semibold text-sm transition-colors";
